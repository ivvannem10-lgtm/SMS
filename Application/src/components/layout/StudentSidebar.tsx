...
import { LayoutDashboard, BookOpen, Receipt, User, Bell, ClipboardList, LogOut, GraduationCap, BarChart2, Inbox, LifeBuoy, ClipboardCheck } from 'lucide-react'
import { cn, initials } from '@/lib/utils'

const NAV = [
  { href: '/student/dashboard',     label: 'Dashboard',            icon: LayoutDashboard },
  { href: '/student/enrollment',    label: 'Enrollment',           icon: ClipboardList },
  { href: '/student/subjects',      label: 'My Subjects',          icon: BookOpen },
  { href: '/student/grades',        label: 'My Grades',            icon: BarChart2 },
  { href: '/student/soa',           label: 'Statement of Account', icon: Receipt },
  { href: '/student/forms',         label: 'Forms',                icon: ClipboardCheck },
  { href: '/student/requests',      label: 'Request Center',       icon: Inbox },
  { href: '/student/support',       label: 'Support Center',       icon: LifeBuoy },
    ...
    <aside className="fixed inset-y-0 left-0 z-40 flex w-[220px] flex-col bg-sidebar-bg">
      ...
      <div className="flex h-14 items-center gap-2.5 px-4 border-b border-sidebar-border">
        ...
        <div className="min-w-0">
          ...
          <p className="text-2xs text-sidebar-text leading-tight">Student Portal</p>
        </div>
      </div>

      {/* ── Navigation ───────────────────────────────────────────────────── */}
      <nav className="flex-1 overflow-y-auto px-2 py-3">
        <p className="mb-1 px-2 text-2xs font-bold uppercase tracking-[0.1em] text-sidebar-heading">Navigation</p>
        <div className="space-y-0.5 mt-1">
          {NAV.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={cn('h-4 w-4 shrink-0 transition-colors', active ? 'text-brand-300' : 'text-sidebar-muted')} />
                <span className="truncate">{item.label}</span>
                {active && <div className="ml-auto h-1.5 w-1.5 rounded-full bg-brand-300" />}
              </Link>
            )
          })}
        </div>
      ...
      <div className="border-t border-sidebar-border p-2 space-y-1">
        <div className="flex items-center gap-2.5 rounded-lg px-2.5 py-2">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-500 text-2xs font-bold text-white">
            {user?.name ? initials(user.name) : 'S'}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[12px] font-semibold text-slate-200">{user?.name ?? 'Student'}</p>
            <p className="text-2xs text-sidebar-text">Student</p>
          </div>
        </div>
        <button
