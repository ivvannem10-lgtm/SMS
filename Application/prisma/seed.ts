...
async function main() {
  ...
  const [, , , , , , , , , , , , , uTeacher, uStudent] = await Promise.all([
  ...
  const sem = existingSem ?? await db.semester.create({
    data: { name: '1st Semester', type: 'FIRST', isActive: true, startDate: new Date('2025-08-01'), endDate: new Date('2025-12-20'), enrollmentStart: new Date('2025-07-01'), enrollmentEnd: new Date('2025-08-15'), maxUnits: 24, academicYearId: ay.id },
  })
  ...
  const faculty = await db.faculty.upsert({
    ...
    update: {},
    create: { facultyId: 'FAC-2024-001', firstName: 'Roberto', lastName: 'Santos', email: 'prof.santos@school.edu', phone: '09171234567', department: 'College of Computing', position: 'Associate Professor', status: 'ACTIVE', schoolId: school.id, userId: uTeacher.id },
  })
  ...
  const off1 = existingOff ?? await db.subjectOffering.create({
    data: {
      section: 'BSCS-1A', status: 'PUBLISHED', maxStudents: 35, subjectId: sCS101.id, semesterId: sem.id,
  ...
  const student = existingStudent ?? await db.student.create({
    data: { studentId: '2025-00000', firstName: 'Ethan', lastName: 'Dela Cruz', email: 'student@school.edu', phone: '09000000000', dateOfBirth: new Date('2005-01-01'), gender: 'MALE', address: 'St. Dominic College', yearLevel: 1, status: 'ACTIVE', programId: progCS.id, schoolId: school.id, userId: uStudent.id },
  })

  // ── Enrollment ────────────────────────────────────────────────────────────────
  const existingEnr = await db.enrollment.findFirst({ where: { studentId: student.id, offeringId: off1.id } })
  if (!existingEnr) {
    await db.enrollment.create({ data: { status: 'ENROLLED', studentId: student.id, offeringId: off1.id, semesterId: sem.id, confirmedAt: new Date('2025-08-12') } })
  }
  ...
  if (!existingSOA) {
    await db.sOA.create({
      data: {
        status: 'UNPAID', totalAmount: 28500, paidAmount: 0, balance: 28500, semesterId: sem.id, studentId: student.id,
        items: { create: [
  ...
  console.log('  (+ 12 more — see /dev for full list)')
}
